const timezoneFlags = {
  ...
  '--update':
    'Allows the user to update the timezone to a new one using the country abbreviation.\nEx: `!timezone --update ES`',
  '--get':
    'Get the current time of the user informed.\nEx: `!timezone --get `<@!939602133005774858>',
  '--sub':
...
const helpFlag = (interaction, flags = timezoneFlags) => {
  let flagsStr =
    'This is the list of all available flags for the `!timezone` command:';
  Object.entries(flags).map((flagDetails) => {
    const [name, description] = flagDetails;
    flagsStr += `\n\n**${name}**: ${description}`;
  });
